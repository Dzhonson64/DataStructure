...
void HeapTree::siftDown(int currentIndex)
{
	...
	while (currentIndex < count) {
		...
		leftIndex = 2 * currentIndex + 1;	// вычисл€ем позицию левого элемента
		rightIndex = 2 * currentIndex + 2;	// вычисл€ем позицию правого элемента
		...
		if (rightIndex < count && tree[rightIndex] > tree[maxIndex]) { // если не вышли за границу и ещЄ и впрвом уезле элемент больше чем i-ый, то присваиваем maxIndex
			maxIndex = rightIndex;
		}
...
{
	while (i > 0 && tree[i] > tree[i / 2]) { // пока массив не закончилс€ и родитель меньше рассматриваемого элемента
		swap(tree[i], tree[i / 2]);			 // мен€ем их местами
		i = i / 2;
	}
}
...
void HeapTree::insert(int num)
{
	...
	int parentIndex = (currentIndex - 1) / 2;	// запоминаем его родител€

	while (currentIndex > 0 && tree[parentIndex] < tree[currentIndex]) { // пока массив не закончилс€ и родитель меньше рассматриваемого элемента
		/* мен€ем их местами */
		int temp = tree[currentIndex];
		tree[currentIndex] = tree[parentIndex];
		tree[parentIndex] = temp;

		currentIndex = parentIndex;				// рассматриваем элементом теперь становитс€ элемент на месте родител€
		parentIndex = (currentIndex - 1) / 2;	// теперь берЄм родител€ относительно нового элемента
	}
...
void HeapTree::siftUpRecursively(int currentIndex)
{
	if (currentIndex > 0 && tree[currentIndex] > tree[currentIndex / 2]) {
		swap(tree[currentIndex], tree[currentIndex / 2]); // мен€ем их местами
...
int HeapTree::pop()
{
	...
	tree.pop_back();	// удал€ем последний элемет массива
	siftDown(0);		// делаем просев нового корн€ вниз
	return result;		// возвращ€ем старый корень
}
